...
function operate(config, paths, o) {
    ...
    function finish(err, data) {
        ...
        writePhotos(tpls.photo, utils.attachAdjacency(photos, 'href'));

        albums.parse(o.output, config.thumbnailPath, thumbnails, function(err, data) {
            if(err) return console.error(err);

            albums.write(config, o.output, tpls.album, data);
            writeIndex(tpls.index, data);
        });
    }

    function writePhotos(tpl, photos) {
    ...
    function writeIndex(tpl, albums) {
