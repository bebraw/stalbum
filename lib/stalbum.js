...
function operate(config, paths, o, albumPath, photoPath) {
    ...
    function iterator(p, cb) {
        ...
        im.identify(ip, function(err, features) {
            ...
            var atPath = mkdir(path.join(thumbnailRoot, album));
    ...
    function finish(err, thumbnails) {
        ...
        parseAlbums(config, thumbnailRoot, thumbnails, function(err, albums) {
            if(err) return console.error(err);
...
function parseAlbums(config, root, thumbnails, cb) {
    var albums = {};

    thumbnails.filter(id).forEach(function(thumbnail) {
        var parts = trim.left(thumbnail, root).split('/');
